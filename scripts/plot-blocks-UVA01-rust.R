...
# Choose a random anchor block present in all genomes once and only once
anchor.block.condition = sapply(unique(genome.blocks$block), function(x) length(genome.blocks$genome[which(genome.blocks$block==x)])==length(unique(genome.blocks$genome)) && 
         length(unique(genome.blocks$genome[which(genome.blocks$block==x)]))==length(unique(genome.blocks$genome)))
anchor.block.candidates = anchor.block.condition[which(anchor.block.condition==TRUE)]
anchor.block <- names(anchor.block.candidates)[1]
