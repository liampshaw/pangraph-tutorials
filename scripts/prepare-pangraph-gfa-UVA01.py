# Converts a gfa to a dataframe of linear blocks
# with consistent colours for bandage vs. linear plotting 
# !!! N.B. Uses block consensus lengths. This may not be correct 
# for blocks with insertions/deletions - but it is OK for this example.
import re
...
def readGFA(gfa_file, colour_seed=1789, colour_unique=False):
    ...
    for block, count in block_count_dict.items():
        if count==1 and colour_unique==False:
...
readGFA(input_gfa, colour_unique=True)
